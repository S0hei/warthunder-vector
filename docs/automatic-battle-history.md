...
Own-unit assignment is established from `MPlayer::onStateChanged()` with `l=1` and the logged account ID. Each matching `UnitRespawn` counts once, including an aircraft's airfield restoration (which can occur without an IN_FLIGHT state change). Timestamp/unit event keys survive rescans and merge reconnects. Landings, repair-start/burn messages, enemy units and former own units are not counted. Replay-only battles have no inferred spawn count. The two ratios use independent coverage: total kills / total deaths, and total kills / logged spawns, only across battles providing both inputs. AI remains separate.
