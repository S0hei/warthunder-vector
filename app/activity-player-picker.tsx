...
export default function ActivityPlayerPicker({ participants = [], status, search, selectedName, onChange, compact = false }: {
  ...
  return <div className="activity-picker" onBlur={event => {
    ...
    <label className="activity-picker-label" htmlFor={id}>Find participant</label>
    ...
    <div className="activity-picker-input">
      <input ref={input} id={id} className="activity-search" type="text" role="combobox" aria-autocomplete="list"
        ...
        placeholder="Name or aircraft" value={search} autoComplete="off" spellCheck={false}
      ...
      {search && <button type="button" aria-label="Clear participant filter" onClick={() => { onChange('', null); setActiveName(null); input.current?.focus(); setOpen(true); }}>×</button>}
      <button type="button" aria-label="Browse participants" aria-expanded={expanded} aria-controls={`${id}-list`}
    ...
    {expanded && <div className="activity-picker-menu">
      <div className="activity-picker-menu-heading">{status === 'live' ? 'This flight' : 'Last flight'}<span>{matches.length}</span></div>
      <ul ref={list} id={`${id}-list`} role="listbox" aria-label="Observed participants">
        {matches.map((participant, index) => <li key={participant.name} id={`${id}-option-${index}`} role="option"
          ...
          <div><strong>{participant.name}</strong><span>{teamLabels[participant.team]}</span></div>
          <AircraftNames vehicles={[participant.vehicle]} />
        </li>)}
      </ul>
      {matches.length === 0 && <p role="status">{search ? 'No matching participants' : 'No participants observed yet'}</p>}
    </div>}
    </div>
    {selected && !compact && <div className={`activity-picked combat-${selected.team}`}>
      <div><strong>{selected.name}</strong><span>{teamLabels[selected.team]}</span></div>
      <span className="activity-picked-label">Last observed aircraft</span>
      <AircraftNames vehicles={[selected.vehicle]} />
      <time dateTime={new Date(selected.observedAt).toISOString()}>{new Date(selected.observedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })}{status !== 'live' ? ' · Last flight' : ''}</time>
    </div>}
  </div>;
}
